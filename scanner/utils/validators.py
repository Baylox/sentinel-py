...
def parse_port_range(ports_range: str) -> Tuple[int, int]:
    ...
    if not (validate_port(start) and validate_port(end)):
        raise PortRangeError(f"Ports must be between 1 and 65535. Got range: {start}-{end}")
    
    if start > end:
        raise PortRangeError(f"Start port ({start}) cannot be greater than end port ({end})")
